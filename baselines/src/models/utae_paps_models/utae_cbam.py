...
class UTAE_CBAM(nn.Module):
    def __init__(
        ...
        # CBAM for encoder and decoder
        self.cbam_encoders = nn.ModuleList([CBAM(encoder_widths[i], r=r) for i in range(self.n_stages - 1)])
        self.cbam_decoders = nn.ModuleList([CBAM(decoder_widths[i], r=r) for i in range(self.n_stages - 1)])
    ...
    def forward(self, input, batch_positions=None, return_att=False):
        ...
        for i in range(self.n_stages - 1):
            ...
            # out = self.cbam_encoders[i](out)  # ADDED CBAM
            # feature_maps.append(out)
            feature_maps.append(self.cbam_encoders[i](out)) # placing CBAM before the skip connection in the encoder
        # TEMPORAL ENCODER
        ...
        for i in range(self.n_stages - 1):
            ...
            out = self.cbam_decoders[i](out)  # ADDED CBAM
            if self.return_maps:
                maps.append(out)
